import fs from 'node:fs';
import { Command } from 'commander';
import { executeCommand } from '../client.js';
import { loadConfig } from '../config.js';
import { printResult, getExitCode } from '../output.js';
import type { GlobalOptions, OutputMode } from '../types.js';

function getOutputMode(opts: GlobalOptions): OutputMode {
  if (opts.json) return 'json';
  if (opts.plain) return 'plain';
  return 'human';
}

function getGlobalOpts(cmd: Command): GlobalOptions {
  let root = cmd;
  while (root.parent) root = root.parent;
  return root.opts() as GlobalOptions;
}

async function run(
  cmd: Command,
  action: string,
  params: Record<string, any>
): Promise<void> {
  const globalOpts = getGlobalOpts(cmd);
  const config = loadConfig(globalOpts);
  const result = await executeCommand('docs', action, params, config, {
    timeout: globalOpts.timeout ? Number(globalOpts.timeout) : undefined,
    maxRetries: globalOpts.retry ? Number(globalOpts.retry) : undefined,
    verbose: globalOpts.verbose,
  });
  printResult(result, getOutputMode(globalOpts), 'docs', action);
  process.exitCode = getExitCode(result);
}

export function registerDocsCommands(program: Command): void {
  const docs = program
    .command('docs')
    .description('Google Docs operations');

  // --- Get metadata ---
  docs
    .command('get <documentId>')
    .description('Get document metadata')
    .action(async (documentId: string, _opts: any, cmd: Command) => {
      await run(cmd, 'get', { documentId });
    });

  // --- Cat (read content) ---
  docs
    .command('cat <documentId>')
    .description('Get document as plain text')
    .option('--max-bytes <n>', 'Maximum bytes to return', '100000')
    .action(async (documentId: string, opts: any, cmd: Command) => {
      await run(cmd, 'cat', {
        documentId,
        maxBytes: Number(opts.maxBytes),
      });
    });

  // --- Create ---
  docs
    .command('create')
    .description('Create a new document')
    .option('--title <text>', 'Document title', 'Untitled Document')
    .option('--content <text>', 'Initial text content')
    .option('--folder <id>', 'Destination folder ID')
    .action(async (opts: any, cmd: Command) => {
      await run(cmd, 'create', {
        title: opts.title,
        content: opts.content,
        folderId: opts.folder,
      });
    });

  // --- Copy ---
  docs
    .command('copy <documentId>')
    .description('Copy an existing document')
    .option('--name <text>', 'Name for the copy')
    .option('--folder <id>', 'Destination folder ID')
    .action(async (documentId: string, opts: any, cmd: Command) => {
      await run(cmd, 'copy', {
        documentId,
        name: opts.name,
        folderId: opts.folder,
      });
    });

  // --- Export ---
  docs
    .command('export <documentId>')
    .description('Export document to format')
    .option('--format <fmt>', 'Export format (pdf, docx, txt, html)', 'pdf')
    .option('--out <path>', 'Output file path')
    .action(async (documentId: string, opts: any, cmd: Command) => {
      const globalOpts = getGlobalOpts(cmd);
      const config = loadConfig(globalOpts);
      const result = await executeCommand('docs', 'export', {
        documentId,
        format: opts.format,
      }, config, {
        timeout: globalOpts.timeout ? Number(globalOpts.timeout) : undefined,
        maxRetries: globalOpts.retry ? Number(globalOpts.retry) : undefined,
        verbose: globalOpts.verbose,
      });

      if (result.ok && result.data) {
        const { content, encoding } = result.data;
        const outPath = opts.out || `document.${opts.format}`;

        if (encoding === 'base64') {
          const buffer = Buffer.from(content, 'base64');
          if (opts.out) {
            fs.writeFileSync(outPath, buffer);
            process.stderr.write(`Exported to ${outPath} (${buffer.length} bytes)\n`);
          } else {
            process.stdout.write(buffer);
          }
        } else {
          if (opts.out) {
            fs.writeFileSync(outPath, content, 'utf-8');
            process.stderr.write(`Exported to ${outPath}\n`);
          } else {
            process.stdout.write(content);
          }
        }
        process.exitCode = 0;
      } else {
        printResult(result, getOutputMode(globalOpts), 'docs', 'export');
        process.exitCode = getExitCode(result);
      }
    });
}
